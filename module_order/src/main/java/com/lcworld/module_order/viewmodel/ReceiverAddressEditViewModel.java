...
public class ReceiverAddressEditViewModel extends BaseViewModelEnhance {
    ...
    //省的值
    public final ObservableField<String> valueProvince = new ObservableField<>("北京");
    public final ObservableInt valueProvinceId = new ObservableInt(1);
    //市的值
    public final ObservableField<String> valueCity = new ObservableField<>("东城区");
    public final ObservableInt valueCityId = new ObservableInt(2802);
    //县的值
    public final ObservableField<String> valueCounty = new ObservableField<>("内环到三环里");
    public final ObservableInt valueCountyId = new ObservableInt(2821);
    //是否是默认地址
    ...
    //所在区域的点击事件
    public final BindingCommand clickOfRegion = new BindingCommand(new BindingAction() {
        @Override
        public void call() {
            valueRegisons.set(valueProvince.get() + valueCity.get() + valueCounty.get());
        }
    });
    //保存按钮的点击事件
    ...
    public ReceiverAddressEditViewModel(@NonNull Application application) {
        ...
        requestMemeberAddresssRegion(0);
    }
    ...
    //请求所在区域
    public void requestMemeberAddresssRegion(int id) {
        RetrofitClient.getInstance().create(ApiServiceInterf.class)
                .regionsIdChildren(id)
                .compose(RxUtilsEnhanced.explicitTransform())
